...
type
  iDAOGeneric<T: Class> = interface
    ...
    function Find(var aList: TObjectList<T>; limit: integer = 20;
      page: integer = 1): iDAOGeneric<T>; overload;
    function Find(const aID: String; var aObject: T): iDAOGeneric<T>; overload;
    function Find(aKey: String; aValue: Variant; var aObject: T)
      : iDAOGeneric<T>; overload;
    function Find(campos, where, orderby, join, groupby: string; var aObject: T)
      : iDAOGeneric<T>; overload;
...
begin
  ...
  FConn := TSimpleQueryFiredac.New(Connection.DbConnection.FConnList.Items
    [FIndexConn]).SQLType(MySQL);
...
function TDAOGeneric<T>.Find(aKey: String; aValue: Variant; var aObject: T)
  : iDAOGeneric<T>;
begin
  Result := Self;
...
function TDAOGeneric<T>.Find(var aList: TObjectList<T>; limit: integer = 20;
  page: integer = 1): iDAOGeneric<T>;
begin
  Result := Self;
...
function TDAOGeneric<T>.Find(campos, where, orderby, join, groupby: string;
